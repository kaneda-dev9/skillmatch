...
- [ ] **Step 1: Supabase 関連パッケージをインストール**

```bash
pnpm add @supabase/supabase-js @supabase/ssr
```

- [ ] **Step 2: AI SDK プロバイダーパッケージをインストール**

```bash
pnpm add @ai-sdk/anthropic @ai-sdk/openai
```

- [ ] **Step 3: Zod をインストール（構造化出力のスキーマ定義用）**
...
- [ ] **Step 2: .gitignore に .env.local が含まれていることを確認**
...
- [ ] **Step 1: クライアントサイド用 Supabase クライアントを作成**
...
- [ ] **Step 2: サーバーサイド用 Supabase クライアントを作成**
...
- [ ] **Step 3: proxy.ts 用のセッション更新ヘルパーを作成**
...
- [ ] **Step 1: proxy.ts を作成（Next.js 16 の認証チェック）**
...
- [ ] **Step 4: ダッシュボードページ（プレースホルダー）を作成**
...
- [ ] **Step 5: ルートページを /dashboard へリダイレクトに変更**
...
- [ ] **Step 6: ルートレイアウトを更新（lang="ja" に変更、メタデータ更新）**
...
- [ ] **Step 1: 初期スキーマのマイグレーション SQL を作成**
...
- [ ] Supabase クライアント（ブラウザ/サーバー/proxy）が設定済み
- [ ] proxy.ts で未認証ユーザーが /login にリダイレクトされる
- [ ] AI SDK Provider Registry（Claude + OpenAI）が設定済み
- [ ] ログイン・サインアップ画面が表示される
- [ ] ダッシュボードレイアウト（サイドバー + ヘッダー）が表示される
- [ ] DB スキーマ（全7テーブル + RLS）のマイグレーション SQL が用意されている
- [ ] `pnpm run build` が通る
